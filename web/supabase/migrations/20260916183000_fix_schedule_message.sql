-- Fix: plpgsql RAISE uses a bare % as its format placeholder, not %s.
-- The previous version left literal "s" characters in the message
-- ("...09s:00s..."). Build the HH:MI labels first, then interpolate once
-- each.
